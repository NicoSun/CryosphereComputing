...
def looop():
	...
	for count in range (0,daycount,1): #366
		filenamedav = 'test/NSIDC_Min_'+str(month).zfill(2)+str(day).zfill(2)+'.bin'
		...
		filename17 = 'DataFiles/NSIDC_2017'+str(month).zfill(2)+str(day).zfill(2)+'.bin'
		...
		try:
			...
			with open(filename17, 'rb') as fr17:
				ice17 = np.fromfile(fr17, dtype=np.uint8)
		...
		for x in range (0,136192):
			icedavf[x] = min(ice7f[x],ice8f[x],ice9f[x],ice10f[x],ice11f[x],ice12f[x],ice13f[x],ice14f[x],ice15f[x],ice16f[x],ice17f[x])
