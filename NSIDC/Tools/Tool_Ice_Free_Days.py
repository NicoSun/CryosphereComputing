...
class NSIDC_area:
	...
	def masksload(self):
		...
		with open(regionmaskfile, 'rb') as frmsk:
			self.regmask = np.fromfile(frmsk, dtype=np.uint32)
	...
	def dayloop(self):
		...
		for count in range (0,self.daycount):
			...
			filename = 'DataFiles/Daily_Mean/NSIDC_Mean_{}{}.bin'.format(self.stringmonth,self.stringday)
#			filename = 'DataFiles/NSIDC_{}{}{}.bin'.format(self.year,self.stringmonth,self.stringday)
	...
	def normalshow(self,icemap):
		...
		ax.set_title('Arctic Ice Free Days: {}'.format(self.year-1),x=0.5)
		...
		fig.savefig('csvexport/Arctic_IceFreeDays_{}.png'.format(self.year-1))
	...
	def anomalyshow(self,icemap,year):
		...
		fig.savefig('csvexport/Arctic_IceFreeDays__anom_{}.png'.format(year))
