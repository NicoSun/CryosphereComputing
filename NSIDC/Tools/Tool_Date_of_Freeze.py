...
class NSIDC_area:
	...
	def dayloop(self):		
		...
		with open(os.path.join(filepath,'Daily_average/NSIDC_Mean_0320.bin'), 'rb') as fr6:
		...
		with open(os.path.join(filepath,'Daily_average/NSIDC_Mean_0914.bin'), 'rb') as fr0: #Minimum,Maximum,Daily_average
				iceminimum = np.fromfile(fr0, dtype=np.uint8)
		for x in range (0,136192):
			if  1 < self.regmaskf[x] < 16:
				if  iceminimum[x] > 37:
					iceminyear[x] = -2
		with open('Special_Animation/'+self.melttype+'_Freeze_Date.bin', 'wb') as writecumu:
