...
class NSIDC_area:
	...
	def dayloop(self):
		...
		with open(os.path.join(filepath,'Minimum/NSIDC_Min_0930_south.bin'), 'rb') as fr:
			iceminyear = np.fromfile(fr, dtype=np.uint8)
			iceminyear = np.array(iceminyear, dtype=float)
		with open(os.path.join(filepath,'Daily_Mean/NSIDC_Mean_0930_south.bin'), 'rb') as fr:
			icemeanyear = np.fromfile(fr, dtype=np.uint8)
			icemeanyear = np.array(icemeanyear, dtype=float)
		with open(os.path.join(filepath,'Maximum/NSIDC_Max_0930_south.bin'), 'rb') as fr:
...
#				icewr = writecumu.write(iceminyear)	
	...
	def normalshow(self,icemap,code):
		...
		if code == 'min':
			fig.savefig('csvexport/South_Earliest_Melt_Date.png')
		elif code == 'mean':
			fig.savefig('csvexport/South_Mean_Melt_Date.png')
		elif code == 'max':
			fig.savefig('csvexport/South_Latest_Melt_Date.png')
#		plt.pause(0.01)
