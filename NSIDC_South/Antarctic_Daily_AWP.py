...
class AWP_calc_daily:
	...
	def masksload(self):
		...
		icemaskfile = 'X:/NSIDC_South/Masks/Max_AWP_extent_south.bin'
...
#		plt.show()
	...
	def daycalc(self,DayofYear):
		...
		filename = 'X:/NSIDC_South/DataFiles/NSIDC_{}{}{}_south.bin'.format(self.year,self.stringmonth,self.stringday)
	#	filenameMean = 'X:/NSIDC/DataFiles/Daily_Mean/NSIDC_Mean_{}{}.bin'.format(self.stringmonth,self.stringday)
		filenameAWPMean = 'X:/NSIDC_South/DataFiles/Mean_00_19/NSIDC_Mean_{}{}_south.bin'.format(self.stringmonth,self.stringday)
		...
		self.AWPdaily,self.AWPcumulative,self.AWPanomaly,self.AWPanomaly_acumu,AWPdaily_areaweighted,AWPcumulative_areaweighted,AWPdaily_oceanarea,AWPcumulative_oceanarea = aaa(DayofYear,icef,
			iceMeanfloat,self.AWPcumulative,self.AWPanomaly_acumu,self.Icemask,self.regmaskf,self.areamaskf,self.latmaskf)
	...
	def normalshow(self,icemap,icesum):
		...
		self.ax.clear()
		self.ax.set_xlabel('Pan Antarctic Mean: '+str(icesum)+' [MJ / 'r'$m^2$]',**self.labelfont)
		...
		self.ax.set_ylabel('cryospherecomputing.tk/awp-south',y=0.2)
		
		cbar = self.fig.colorbar(cax, ticks=[0,cbarmax*0.25,cbarmax*0.5,cbarmax*0.75
			,cbarmax]).set_label('clear sky energy absorption in [MJ / 'r'$m^2$]',**self.labelfont)
		
		self.fig.tight_layout()
		plt.pause(0.01)
		self.fig.savefig('X:/Upload/AWP/South_AWP_Map1.png')
	...
	def cumulativeshow(self,icemap,icesum):
		...
		self.ax2.clear()
		self.ax2.set_xlabel('Pan Antarctic Mean: '+str(int(icesum))+' [MJ / 'r'$m^2$]',**self.labelfont)
		...
		self.ax2.set_ylabel('cryospherecomputing.tk/awp-south',y=0.2)
		...
		plt.pause(0.01)
		self.fig2.savefig('X:/Upload/AWP/South_AWP_Map2.png')
	...
	def anomalyshow(self,icemap,icesum):
		...
		ax.set_xlabel('Pan Antarctic Anomaly: '+str(icesum)+' [MJ / 'r'$m^2$]',**self.labelfont)
		ax.set_ylabel('cryospherecomputing.tk/awp-south',y=0.2)
		...
		fig.tight_layout()
		plt.pause(0.01)
		fig.savefig('X:/Upload/AWP/South_AWP_Map3.png')
		plt.close()
	...
	def anomalyshow_accu(self,icemap,icesum):
		...
		fig.tight_layout()
		plt.pause(0.01)
		fig.savefig('X:/Upload/AWP/South_AWP_Map4.png')
		plt.close()
	...
	def createMaps(self,AWP_Daily_mean,AWP_Accu_mean):
		'''creates Maps'''
	...
	def save_data(self):
		self.csvexport('X:/Upload/AWP_data/South_AWP_NRT.csv',
				[self.CSVDatum,self.CSVDaily,self.CSVCumu])
		self.csvexport('X:/Upload/AWP_data/South_AWP_NRT_regional.csv',
				[self.CSVDatum,self.Wed,self.Ind,self.Pac,self.Ross,self.Bell])
		self.csvexport('X:/Upload/AWP_data/South_AWP_NRT_regional_daily.csv',
				[self.CSVDatum,self.Wed_daily,self.Ind_daily,self.Pac_daily,self.Ross_daily,self.Bell_daily])
		
		#save binary accumulative files
		with open('X:/Upload/AWP_data/South_AWP_Accu.bin', 'wb') as writecumu:
			writecumu.write(self.AWPcumulative)
		with open('X:/Upload/AWP_data/South_AWP_Accu_anom.bin', 'wb') as writecumu:
	...
	def loadCSVdata (self):
		...
		AWP_mean = ['A','B']
		Climatedata = pandas.read_csv('X:/Upload/AWP_data/Climatology/South_AWP_mean.csv', names=AWP_mean)
	...
	def automated (self,year,stringmonth,stringday):
		...
		with open('X:/Upload/AWP_data/South_AWP_Accu.bin', 'rb') as readcumu:
			self.AWPcumulative = np.fromfile(readcumu, dtype=float)
		with open('X:/Upload/AWP_data/South_AWP_Accu_anom.bin', 'rb') as readcumu:
			self.AWPanomaly_acumu = np.fromfile(readcumu, dtype=float)
		
		index = len(self.CSVDaily)
#		print(self.AWP_Daily_mean[index])
		self.daycalc(index)
		self.createMaps(self.AWP_Daily_mean[index],self.AWP_Accu_mean[index])
		self.save_data()
