...
class NSIDC_area:
	...
	def __init__  (self):
		self.start = date(2012, 3, 3)
		self.month = self.start.month
		self.day = self.start.day
		self.daycount = 155 #155 (Oct-Feb)
		#01/04 day 92
		#31/08 day 244
	...
	def dayloop(self):		
		...
		with open(os.path.join(filepath,'Daily_average/NSIDC_Mean_0930_south.bin'), 'rb') as fr6:
		...
		for x in range (0,104912):
			if  iceminyear[x] == 500:
				iceminyear[x] = -2
		with open('tempgif/South_'+self.melttype+'_Melt_Date.bin', 'wb') as writecumu:
	...
	def normalshow(self,icemap):		
		...
		ax.set_title(self.melttype+' Melt Date') #Earliest, Median, Latest Date without Ice cover
		...
		fig.savefig('tempgif/South_'+self.melttype+'_Melt_Date.png')
		plt.pause(0.01)		
	
	
	def combograph (self):
		
		with open('tempgif/South_Earliest_Melt_Date.bin', 'rb') as aaa:
			early = np.fromfile(aaa, dtype=np.float)
				
		with open('tempgif/South_Median_Melt_Date.bin', 'rb') as bbb:
			median = np.fromfile(bbb, dtype=np.float)
				
		with open('tempgif/South_Latest_Melt_Date.bin', 'rb') as ccc:
		...
		map1 = ma.masked_outside(early,200,450) 
		map11 = ma.masked_outside(early,-3,+2)
		map2 = ma.masked_outside(median,200,450) 
		map21 = ma.masked_outside(median,-3,+2)
		map3 = ma.masked_outside(late,200,450) 
		map31 = ma.masked_outside(late,-3,+2)
		
		
		fig = plt.figure(figsize=(17, 6))
		fig.suptitle('Antarctic Melt Dates', fontsize=14, fontweight='bold')
		...
		fig.savefig('tempgif/Combo_South_Melt_Date.png')
		plt.show()
