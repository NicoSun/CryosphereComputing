...
def looop():
	...
	for count in range (0,daycount,1): #372
		filenamedav = 'Minimum/NSIDC_Min_'+str(month).zfill(2)+str(day).zfill(2)+'_south.bin'
		...
		icedminf = np.zeros(104912, dtype=float)
		...
		for x in range (0,104912):
			icedminf[x] = min(ice6f[x],ice7f[x],ice8f[x],ice9f[x],ice10f[x],ice11f[x],ice12f[x],ice13f[x],ice14f[x],ice15f[x],ice16f[x])
		
		
		export = np.array(icedminf, dtype=np.uint8)
