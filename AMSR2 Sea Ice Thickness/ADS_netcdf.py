...
#		times = rootgrp.createVariable("time","f4",("time",))
		...
		icecthickness = rootgrp.createVariable("siconc","u2",("time","xaxis","yaxis",),zlib=True)
		...
		while x < self.daycount:
			filepath = 'X:/ADS/Binary'
			filename = 'AMSR2_SIT_{}{}{}.dat'.format(self.year,self.stringmonth,self.stringday)
			with open(os.path.join(filepath,filename), 'rb') as frr:
				icethickness_load = np.fromfile(frr, dtype=np.uint16)
