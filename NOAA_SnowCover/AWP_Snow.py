...
class AWP_model:
	...
	def __init__  (self):
		...
		self.daycount = 21*365 #366year, 181 austral summe
		self.masksload()
...
# =============================================================================
	...
	def init_regionlists(self):
		...
		self.NorthAmerica = [['Canada','USA','eastern Canada','western Canada','northern Canada',
		...
		self.Asia = [['Asia','East Siberia','Central Siberial','West Siberia','Mid-lat Asia','Tibet']]
		
		self.Ocean_anom = ['Ice-Ocean anomaly']
		self.NorthAmerica_anom = [['Canada','USA','eastern Canada','western Canada','northern Canada',
					'Alaska','US NE','US MidWest','US Pacific','US Rockies']]
		...
		self.Asia_anom = [['Asia','East Siberia','Central Siberial','West Siberia','Mid-lat Asia','Tibet']]
...
# =============================================================================
# 		Mask = self.regionmask.reshape(610,450)
# 		plt.imshow(Mask)
...
# =============================================================================
	...
	def listsappend(self):
		...
		#AWP lists
		calclist = [self.Eastern_Canada_calc,self.Western_Canada_calc,self.Northern_Canada_calc]
		arealist = [self.Eastern_Canada_area,self.Western_Canada_area,self.Northern_Canada_area]
		calclist = sum([sum(b) for b in calclist])
		arealist = sum([sum(b) for b in arealist])
		Canada = round(calclist/arealist,3)
		
		calclist = [self.Alaska_calc,self.US_NE_calc,self.US_MW_calc,self.US_Pacific_calc,self.US_Rocki_calc]
		...
		Western_Canada = round((np.sum(self.Western_Canada_calc)/np.sum(self.Western_Canada_area)),3)
		...
		self.NorthAmerica.append([Canada,USA,Eastern_Canada,Western_Canada,Northern_Canada_area,Alaska,US_NE,US_MW,US_Pacific,Rocki])
		...
		calclist = [self.East_sib_calc,self.Cent_sib_calc,self.West_sib_calc,self.mid_Asia_calc,self.Tibet_calc]
		...
		calclist = [self.Eastern_Canada_calc_anom,self.Western_Canada_calc_anom,self.Northern_Canada_calc_anom]
		arealist = [self.Eastern_Canada_area,self.Western_Canada_area,self.Northern_Canada_area]
		calclist = sum([sum(b) for b in calclist])
		arealist = sum([sum(b) for b in arealist])
		Canada = round(calclist/arealist,3)
		...
		Western_Canada = round((np.sum(self.Western_Canada_calc_anom)/np.sum(self.Western_Canada_area)),3)
		...
		self.NorthAmerica_anom.append([Canada,USA,Eastern_Canada,Western_Canada,Northern_Canada_area,Alaska,US_NE,US_MW,US_Pacific,Rocki])
...
#			filenameyesterday = 'X:/SnowCover/Datafiles/NOAA_{}{}_24km.bin'.format(self.year,self.stringstringyearday2)
			...
			bbb(AWPdaily,AWP_anomaly,self.regionmask,self.SubRegion_Mask,self.pixelarea,self.Snow_Mask)
...
# =============================================================================
	...
	def regional_AWP(self,AWPdaily,AWP_anomaly,regmask,subregmask,areamask,Snowmask):
		if Snowmask==5:
			if regmask == 1:
			...
			if subregmask == 10:
				...
				self.Western_Canada_calc.append(AWPdaily*areamask)
				self.Western_Canada_calc_anom.append(AWP_anomaly*areamask)
				self.Western_Canada_area.append(areamask)
			elif subregmask == 18:
			...
			#Asia regions
				...
				self.mid_Asia_calc.append(AWPdaily*areamask)
				self.mid_Asia_calc_anom.append(AWP_anomaly*areamask)
				self.mid_Asia_area.append(areamask)
			elif subregmask == 38:
		...
		with open('CSVexport/AWP_NorthAmerica.csv', "w") as output:
			...
			for x in range(0,(len(self.CSVDatum))):
				writer.writerow([self.CSVDatum[x],self.Greenland[x],self.NorthAmerica[x][0],self.NorthAmerica[x][1],self.NorthAmerica[x][2],
				 ...
				 self.NorthAmerica[x][7],self.NorthAmerica[x][8],self.NorthAmerica[x][9]])
		with open('CSVexport/AWP_NorthAmerica_anomaly.csv', "w") as output:
			...
			for x in range(0,(len(self.CSVDatum))):
				writer.writerow([self.CSVDatum[x],self.Greenland_anom[x],self.NorthAmerica_anom[x][0],self.NorthAmerica_anom[x][1],self.NorthAmerica_anom[x][2],
				 ...
				 self.NorthAmerica_anom[x][7],self.NorthAmerica_anom[x][8],self.NorthAmerica_anom[x][9]])
		...
		with open('CSVexport/AWP_Asia.csv', "w") as output:
			...
			for x in range(0,(len(self.CSVDatum))):
				writer.writerow([self.CSVDatum[x],self.Asia[x][0],self.Asia[x][1],self.Asia[x][2],
				 self.Asia[x][3],self.Asia[x][4],self.Asia[x][5]])
		with open('CSVexport/AWP_Asia_anom.csv', "w") as output:
			...
			for x in range(0,(len(self.CSVDatum))):
				writer.writerow([self.CSVDatum[x],self.Asia_anom[x][0],self.Asia_anom[x][1],self.Asia_anom[x][2],
				 self.Asia_anom[x][3],self.Asia_anom[x][4],self.Asia_anom[x][5]])
