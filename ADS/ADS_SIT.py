...
class ADS_data:
	...
	def __init__  (self):
		...
		self.daycount = 60 #total length: 2158 
...
#			filepath = 'Datafiles/{}{}'.format(self.year,self.stringmonth)
			...
			icethickess,freezedays = aaaaa(icenewdate,icethickess,freezedays)
...
# =============================================================================
	...
	def calculateThickness(self,icenewdate,icethickess,freezedays):
		...
		# melt & freeze algorithm				
			...
			if freezedays < 1:
				...
				#self.meltcount[x] = self.meltcount[x]+5*(icenewdate-1000) # 0.2 melt == one melt day (0.2*5=1), 0.4 melt == 2 melt days
				freezedays = -1						
			...
			#calculates freezing after first refreeze	
				...
				freezedays = freezedays+1 # refreeze days
				#self.freezecount[x] = self.freezecount[x]+1
...
#		plainmap = icemap
	...
	def automated (self,day,month,year,daycount):
		...
		with open(lastmonthdata, 'rb') as fr:
			icethickess = np.fromfile(fr, dtype=float)
